...
//----------------------------------------------------------------------
{
  ...
  def("tai", function(list) {
    ...
    if (!sawEnd) {
      throw new Error(format(__("Expected taškas")));
    }
  ...
  def(["dėk.pirmu", "dpr"], function(thing, list) {
    var l = lexpr(list); l.unshift(thing); return sifw(list, l);
  });

  def(["dėk.paskutinų", "dps"], function(thing, list) {
    var l = lexpr(list); l.push(thing); return sifw(list, l);
  });

  def("masyvas", function(size) {
    size = aexpr(size);
  ...
  def("sąrašas.į.masyvą", function(list) {
    list = lexpr(list);
    var origin = 1;
    if (arguments.length > 1) {
      origin = aexpr(arguments[1]);
  ...
  def("masyvas.į.sarašą", function(array) {
    if (Type(array) !== 'array') {
  ...
  def("sumaišyk", function(thing1, thing2) {
    if (Type(thing2) !== 'list') {
      return self.routines.get('žodis')(thing1, thing2);
    } else {
      return self.routines.get('dėk.pirmu')(thing1, thing2);
    }
  });

  def(".atvirkščiai", function(list) {
  ...
  def("pirmas", function(list) { return lexpr(list)[0]; });

  def("pirmi", function(list) {
    return lexpr(list).map(function(x) { return x[0]; });
  });

  def("paskutinis", function(list) { list = lexpr(list); return list[list.length - 1]; });

  def(["be.pirmo", "bpr"], function(list) {
    return sifw(list, lexpr(list).slice(1));
  });

  def(["be.pirmų", "bprm"], function(list) {
    return lexpr(list).map(function(x) { return sifw(x, lexpr(x).slice(1)); });
  });

  def(["be.paskutinio", "bps"], function(list) {
    return Type(list) === 'word' ? String(list).slice(0, -1) : lexpr(list).slice(0, -1);
  });

  def("dalis", function(index, thing) {
    index = aexpr(index);
    switch (Type(thing)) {
    case 'list':
  ...
  def("keisk", function(index, array, value) {
    index = aexpr(index);
    if (Type(array) !== 'array')
      throw new Error(__("Expected array"));
  ...
  def(["wordp", "žodis?"], function(thing) { return Type(thing) === 'word' ? 1 : 0; });
  def(["listp", "sąrašas?"], function(thing) { return Type(thing) === 'list' ? 1 : 0; });
  def(["arrayp", "masyvas?"], function(thing) { return Type(thing) === 'array' ? 1 : 0; });
  def(["numberp", "skaičius?"], function(thing) {
    return Type(thing) === 'word' && isNumber(thing) ? 1 : 0;
  });
  def(["numberwang"], function(thing) { return self.prng.next() < 0.5 ? 1 : 0; });

  def(["equalp", "lygūs?"], function(a, b) { return equal(a, b) ? 1 : 0; });
  def(["notequalp", "nelygūs?"], function(a, b) { return !equal(a, b) ? 1 : 0; });

  def(["tuščia?", "tuščias?"], function(thing) {
    switch (Type(thing)) {
  ...
  def("ženklas", function(chr) { return sexpr(chr).charCodeAt(0); });
  ...
  def("liekana", function(num1, num2) {
  ...
  def("šaknis", function(a) { return Math.sqrt(aexpr(a)); });
  ...
  def("apvalink", function(a) { return Math.round(aexpr(a)); });
  ...
  def(["greaterp", "didesnis?"], function(a, b) { return aexpr(a) > aexpr(b) ? 1 : 0; });
  def(["greaterequalp", "greaterequal?"], function(a, b) { return aexpr(a) >= aexpr(b) ? 1 : 0; });
  def(["lessp", "mažesnis?"], function(a, b) { return aexpr(a) < aexpr(b) ? 1 : 0; });
  def(["lessequalp", "lessequal?"], function(a, b) { return aexpr(a) <= aexpr(b) ? 1 : 0; });

  // 4.3 Random Numbers

  def("bet.koks", function(max) {
    max = aexpr(max);
  ...
  def("tiesa", function() { return 1; });
  def("melas", function() { return 0; });

  def("ir", function(a, b) {
    return Array.from(arguments).every(function(f) { return f(); }) ? 1 : 0;
  }, {noeval: true});

  def("arba", function(a, b) {
  ...
  def(["priekin", "pr"], function(a) { turtle.move(aexpr(a)); });
  def(["atgal", "at"], function(a) { turtle.move(-aexpr(a)); });
  def(["kairėn", "kr"], function(a) { turtle.turn(-aexpr(a)); });
  def(["dešinėn", "dš"], function(a) { turtle.turn(aexpr(a)); });
  ...
  def("eik.į", function(l) {
    l = lexpr(l);
    if (l.length !== 2) { throw new Error(__("Expected list of length 2")); }
    turtle.setposition(aexpr(l[0]), aexpr(l[1]));
  });
  def("eik.x.y", function(x, y) { turtle.setposition(aexpr(x), aexpr(y)); });
  def("eik.x", function(x) { turtle.setposition(aexpr(x), undefined); }); // TODO: Replace with ...?
  def("eik.y", function(y) { turtle.setposition(undefined, aexpr(y)); });
  def(["setheading", "seth"], function(a) { turtle.setheading(aexpr(a)); });

  def("namo", function() { turtle.home(); });
  ...
  def("koks.x.y", function() { var l = turtle.getxy(); return [l[0], l[1]]; });
  def("koks.x", function() { var l = turtle.getxy(); return l[0]; });
  def("koks.y", function() { var l = turtle.getxy(); return l[1]; });
  def("kryptis", function() { return turtle.getheading(); });
  ...
  def(["rodykis"], function() { turtle.showturtle(); });
  def(["slėpkis"], function() { turtle.hideturtle(); });
  def("valyk", function() { turtle.clear(); });
  def(["valyk.viską", "vv"], function() { turtle.clearscreen(); });
  ...
  def("nuspalvink", function(fillcolor, statements) {
  ...
  def("raštas", function(a) {
    var s = Array.from(arguments).map(stringify_nodecorate).join(" ");
    turtle.drawtext(s);
  });

  def("teksto.dydis", function(a) { turtle.setfontsize(aexpr(a)); });
  ...
  //
  def(["piešim", "pš"], function() { turtle.pendown(); });
  def(["eisim", "es"], function() { turtle.penup(); });
  ...
  def(["pasirink.pieštuko.spalvą", "ppsp"], function(color) {
    function adjust(n) {
      // Clamp into 0...99
  ...
  def(["pasirink.pieštuko.storį", "pps"], function(a) {
    if (Type(a) === 'list') {
  ...
  def(["pieštuko.spalva", "ps"], function() {
    return turtle.getcolor();
  });

  // Not Supported: palette

  def("pieštuko.storis", function() {
  ...
  def("tebus", function(varname, value) {
  ...
  def(["procedurep", "komanda?"], function(name) {
    name = sexpr(name);
  ...
  def(["definedp", "aprašytas?"], function(name) {
    name = sexpr(name);
    return (self.routines.has(name) &&
            !self.routines.get(name).primitive) ? 1 : 0;
  });

  def(["namep", "vardas?"], function(varname) {
    try {
  ...
  def("vykdyk", function(statements) {
    statements = reparse(lexpr(statements));
  ...
  def("kartok", function(count, statements) {
    count = aexpr(count);
    statements = reparse(lexpr(statements));
  ...
  def("jei", function(test, statements) {
    test = aexpr(test);
    statements = reparse(lexpr(statements));
  ...
  def(["jei.tiesa", "ift"], function(statements) {
    statements = reparse(lexpr(statements));
    var tf = self.scopes[self.scopes.length - 1]._test;
    if (tf) { self.execute(statements); }
  });

  def(["jei.melas", "iff"], function(statements) {
    statements = reparse(lexpr(statements));
    var tf = self.scopes[self.scopes.length - 1]._test;
    if (!tf) { self.execute(statements); }
  });
  ...
  def("ciklas", function(control, statements) {
  ...
  def("kiekvienam", function(procname, list) {
    procname = sexpr(procname);
  ...
  def("filtras", function(procname, list) {
    procname = sexpr(procname);
